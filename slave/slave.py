...
def get_mac_address():  # Función 1
    mac = uuid.UUID(int=uuid.getnode()).hex[-12:]
    return ":".join([mac[i:i+2] for i in range(0, 12, 2)])

def receive_file_via_scp(username, password, ip, port, remote_path, local_path):  # Función 2
    try:
...
def calculate_blake2(file_path):  # Función 3
...
def calculate_sha512(file_path):  # Función 4
...
def calculate_sha384(file_path):  # Función 5
...
def decrypt_message_with_rsa(encrypted_message, private_key):  # Función 6
...
def clear_directory(path):  # Función 7
...
def calculate_file_hash(file_path):  # Función 8
    sha512_hash = hashlib.sha512()
    with open(file_path, "rb") as f:
        for byte_block in iter(lambda: f.read(4096), b""):
            sha512_hash.update(byte_block)
...
def handle_request(conn, addr, public_key, private_key, slave_path, scp_username, scp_password):  # Función 9
    try:
        while True:
            request = conn.recv(1024).decode('utf-8')  # Subfunción 9.1
            ...
            if request == "SEND_PUBLIC_KEY":
                conn.send(public_key.encode('utf-8'))  # Subfunción 9.2
                print("Clave pública enviada.")
                mac_address = get_mac_address()  # Subfunción 9.3
                ...
                if os.listdir(slave_path):
                    ...
                    if clear_dir.lower() == 'yes':
                        clear_directory(slave_path)  # Subfunción 9.4
                    else:
                ...
                if decision.lower() == "yes":
                    ...
                    for file_name in file_names:
                        ...
                        try:
                            receive_file_via_scp(scp_username, scp_password, addr[0], 22, remote_path, local_path)  # Subfunción 9.5
                        ...
                        for file_name, expected_hash in received_hashes.items():
                            local_path = os.path.join(slave_path, file_name)
                            calculated_hash = calculate_file_hash(local_path)  # Subfunción 9.6
                            ...
                            conn.send(b"FILES_RECEIVED")  # Subfunción 9.7
                            steghide_password = conn.recv(1024).decode('utf-8')  # Subfunción 9.8
                            ...
                            blake2_calculated = calculate_blake2(os.path.join(slave_path, "stego_file"))  # Subfunción 9.9
                            ...
                            os.system(f"steghide extract -sf {os.path.join(slave_path, 'stego_file')} -p '{steghide_password}' -xf {os.path.join(slave_path, 'extracted_message')}")  # Subfunción 9.10
                            ...
                            sha512_calculated = calculate_sha512(os.path.join(slave_path, "encriptado_con_RSA"))  # Subfunción 9.11
                            ...
                            decrypted_message = decrypt_message_with_rsa(encrypted_message, private_key)  # Subfunción 9.12
                            ...
                            sha384_calculated = calculate_sha384(os.path.join(slave_path, "mensaje_descifrado.txt"))  # Subfunción 9.13
